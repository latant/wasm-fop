...
wasmImports.jsbody = {
	...
	'_JSSymbol.referenceEquals___JSSymbol_JSSymbol_JSBoolean' : (...args) => (function(sym0,sym1){
		try{
			...
			conversion.handleJSError(e);}}).call(...args),
	'_LFopMain.setFileRemover___Consumer_V' : (...args) => (function(func){
		try{
			FOP_FILE_REMOVER = func;
		}catch( e ) {
			conversion.handleJSError(e);}}).call(...args),
	'_LFopMain.setFileSetter___Consumer_V' : (...args) => (function(func){
		try{
			FOP_FILE_SETTER = func;
		}catch( e ) {
			conversion.handleJSError(e);}}).call(...args),
	'_LFopMain.setFilesClearer___Runnable_V' : (...args) => (function(func){
		try{
			FOP_FILES_CLEARER = func;
		}catch( e ) {
			conversion.handleJSError(e);}}).call(...args),
	'_LFopMain.setResult___String_V' : (...args) => (function(res){
		try{
			FOP_RESULT = res;
		}catch( e ) {
			conversion.handleJSError(e);}}).call(...args),
	'_LFopMain.setTransformer___Consumer_V' : (...args) => (function(func){
		try{
			FOP_TRANSFORMER = func;
		}catch( e ) {
			conversion.handleJSError(e);}}).call(...args),
...
return {
    ...
    setFile(name, uint8array) {
        const decoder = new TextDecoder('iso-8859-1');
        const content = decoder.decode(uint8array);
        FOP_FILE_SETTER({name, content});
    },
    ...
    transform(mainFileName) {
        FOP_TRANSFORMER(mainFileName);
        const result = FOP_RESULT;
        FOP_RESULT = undefined;
        return new Uint8Array(result.split('').map(char => char.charCodeAt(0)));
    },
};
